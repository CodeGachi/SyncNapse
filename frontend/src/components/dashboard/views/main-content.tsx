  ...
  return (
    <div className="flex flex-col w-full h-screen">
          ...
          <div className={`flex flex-row items-center px-4 py-2.5 gap-3 w-[320px] h-[48px] bg-background-surface/60 backdrop-blur-xl border rounded-full shadow-lg shadow-black/20 transition-all duration-300 group ${
            ...
            {searchQuery && (
              <button
                ...
                className="w-5 h-5 flex items-center justify-center text-foreground-tertiary hover:text-foreground transition-colors"
              >
      ...
      <div className="flex flex-col items-start px-9 py-12 gap-12 flex-1 bg-background-deep overflow-y-auto pt-24">
        ...
        >
          <h2 className="text-foreground font-bold text-xl leading-6">
            최근 접근한 노트
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full">
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-foreground-tertiary bg-background-surface/40 border border-border-subtle rounded-2xl w-full col-span-full backdrop-blur-sm">
                ...
                >
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <button
                      ...
                      className="p-1.5 text-foreground-tertiary hover:text-foreground hover:bg-foreground/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                    >
                  ...
                  <h3 className="text-foreground font-bold text-lg mb-1 truncate group-hover:text-brand transition-colors">
                    {note.title}
                  </h3>
        ...
        >
          ...
          {/* 구분선 */}
          <div className="w-full h-0 border-t border-border-subtle" />

          {/* 테이블 행 */}
          {childFolders.length === 0 && folderNotes.length === 0 ? (
            <div className="px-5 py-4 text-foreground-tertiary w-full">
              이 폴더가 비어있습니다
            </div>
          ) : (
            <>
              ...
              {folderNotes.map((note) => (
                <div key={note.id} className="w-full">
                  <div
                    onClick={() => handleNoteClick(note)}
                    className="flex flex-row items-center px-5 gap-6 w-full h-10 cursor-pointer hover:bg-background-elevated transition-colors"
                  >
                    {/* 노트 아이콘 + 타입 배지 + 이름 */}
      ...
      {optionMenu && (
        <>
          ...
          <div
            className="fixed z-50 w-40 bg-background-modal/90 border border-border-subtle shadow-xl shadow-black/50 backdrop-blur-xl rounded-xl py-1"
            style={{
          ...
          >
            ...
            <button
              ...
              className="w-full px-4 py-2.5 text-left text-sm text-status-error hover:bg-status-error/10 hover:text-status-error/80 transition-colors flex items-center gap-2"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
