  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-6 w-full">
        ...
        <div className="flex items-center gap-4 text-sm text-foreground-tertiary">
          <div className="flex items-center gap-2">
            <span className="text-foreground-tertiary">위치</span>
            <button
              type="button"
              onClick={() => setIsFolderSelectorOpen(true)}
              className="flex items-center gap-1.5 hover:bg-foreground/5 px-2 py-1 rounded transition-colors text-foreground-secondary hover:text-foreground"
            >
              ...
              <span className="underline decoration-foreground-tertiary underline-offset-2 hover:decoration-foreground-secondary">
                {getSelectedFolderName()}
              </span>
            </button>
          </div>
      ...
      {validationErrors.length > 0 && (
        <div className="w-full bg-status-error/10 border border-status-error/20 rounded-lg p-3">
      ...
      >
        ...
        {uploadedFiles.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-foreground-tertiary gap-4">
            ...
            <div className="text-center">
              <p className="text-lg font-medium text-foreground-tertiary">파일을 여기에 드래그하세요</p>
              <p className="text-sm text-foreground-tertiary/60 mt-1">또는 클릭하여 업로드</p>
            </div>
            <button
              onClick={() => fileInputRef.current?.click()}
              className="mt-2 px-4 py-2 bg-foreground/10 hover:bg-foreground/20 text-foreground text-sm rounded-lg transition-colors"
            >
              파일 선택
            </button>
          </div>
          ...
          <div className="w-full flex flex-col gap-2 max-h-[250px] overflow-y-auto pr-2">
            {uploadedFiles.map((uploadedFile, index) => (
              <div key={`${uploadedFile.file.name}-${index}`} className="flex items-center gap-3 p-3 bg-foreground/5 rounded-xl group hover:bg-foreground/10 transition-colors">
                <div className="w-10 h-10 rounded-lg bg-foreground/10 flex items-center justify-center flex-shrink-0">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-foreground-secondary">
                    <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" />
                    <polyline points="14 2 14 8 20 8" />
                  </svg>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-foreground-secondary truncate">{uploadedFile.file.name}</p>
                  <p className="text-xs text-foreground-tertiary">{(uploadedFile.file.size / 1024 / 1024).toFixed(2)} MB</p>
                </div>
                <button
                  onClick={() => removeUploadedFile(uploadedFile.file)}
                  className="p-2 text-foreground-tertiary hover:text-status-error hover:bg-status-error/10 rounded-lg transition-colors opacity-0 group-hover:opacity-100"
                >
            ...
            <button
              onClick={() => fileInputRef.current?.click()}
              className="mt-2 flex items-center justify-center gap-2 p-3 border border-dashed border-foreground/20 rounded-xl text-foreground-tertiary hover:text-foreground hover:border-foreground/40 hover:bg-foreground/5 transition-all"
            >
